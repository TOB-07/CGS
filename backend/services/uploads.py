...
class TheEventHandler(FileSystemEventHandler):
    ...
    def on_created(self, event: FileSystemEvent, verbose = True):
        try:
            ...
            if not event.is_directory:
                ...
                # folder_path = Path(event.src_path).parent.name => MGS

                folder_name = Path(event.src_path).name #GTA V

                # fe.explore_folder(Path(folder_path)) => No need of file explorer work here
                # if verbose:
                #     print("From FE:")
            ...
            print("Execption in on_created function:")
            traceback.print_exc()
    ...
    def on_modified(self, event: FileSystemEvent, verbose = True):
        try:
            ...
            print("Exception in on_modified function:")
            traceback.print_exc()
    ...
    def on_moved(self, event: FileSystemEvent, verbose = True):
        try:
            ...
            if not event.is_directory:
                ...
                if verbose:
                    print(f"file moved: {event.src_path} -> {event.dest_path} in seaweedfs")
                    print(f"Changed file path accordingly in database {event.src_path} -> {event.dest_path}")

            else:
                folder_name = Path(event.dest_path).name
            ...
            print("Exception in on_moved function:")
            traceback.print_exc()
    ...
    def on_deleted(self, event: FileSystemEvent, verbose = True):
        try:
            ...
            if not event.is_directory:
                ...
                future.result()
                if verbose:
                    print(f"Deleted file: {event.src_path} from seaweedfs")
                    print(f"Deleted file - {event.src_path} changes transferred to database")
                    print("------------------------------------------")
            else:
                asyncio.run(delete_folder(event.src_path))
                game_name = Path(event.src_path).name
                game_id_future = asyncio.run_coroutine_threadsafe(db_actions.get_game_id(game_name),self.loop)
                game_id = game_id_future.result()
                ...
                future.result()
                if verbose:
                    print(f"Deleted folder: {event.src_path} from seaweedfs")
                    print(f"Deleted folder - {game_name} changes transferred to database")
                    print("------------------------------------------")
        except Exception:
            print("Exception in on_deleted function:")
            traceback.print_exc()
